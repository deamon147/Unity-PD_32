...
public class LevelsManager : MonoBehaviour {
	...
	public void SpawnLevel () {
		...
		Destroy (oldLevel); //Удаляет старый уровень
		if (currentLevel == 2) { oldLevel = GameObject.Find ("Level_Start"); }
		else { oldLevel = curLevel; } //Перемещает со старого лвла на новый
		SetOrderLayer (oldLevel, "OldLevel");

		curLevel = newLevel; //Перемещает с нового лвла на старый
		SetOrderLayer (newLevel, "CurrentLevel");

		int randomLvl = Random.Range (0, levelPrefabs.Length);
		newLevel = Instantiate (levelPrefabs [randomLvl], nextLevelPos, Quaternion.identity); //Делает новый уровень
